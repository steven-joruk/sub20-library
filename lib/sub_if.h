...
/*
*-------------------------------------------------------------------
* $Id: sub_if.h,v 1.66 2012-02-29 15:04:23 avr32 Exp $
*-------------------------------------------------------------------
*/
...
typedef struct 
{
	uint8_t		code;
		...
		#define SUB_CNTR_CONFIG		0x5A
		#define SUB_CNTR_READ		0x5B
	...
	union
	{
        ...
        /* EEP read/write */
#define SUB_MAX_EEPRW_SZ 64
        struct eep_rw_req_st
        {
            uint8_t     r_w;    /* 1-Read */
            uint8_t     addr_lo;
            uint8_t     addr_hi;
...
#define SUB_MAX_I2C_WRITE (SUB_MAX_PACK-1-offsetof(sub_tag_t,i2c_rdwr_req.ma))
		...
		/* I2C Read/Write HS (High Speed) */
		struct i2c_hs_rw_req_st
		{
			uint8_t		in_sz;
			uint8_t		master_code;
			uint8_t		n;
...
#define SUB_MAX_I2C_HS_RW_REQ_LONG (SUB_MAX_PACK_LONG-1-offsetof(sub_tag_t,i2c_hs_rw_req.xfer[0]))

		struct i2c_hs_rw_resp_st
		{
			uint8_t		hs_stat;	/* HS mode enter/exit status */
			uint8_t		n;
			struct i2c_hs_stat
			{
				uint8_t	status;
				uint8_t	sz;	  		/* Actual R/W size */
				uint8_t	data[1];	/* If actual size<req.sz will be padding */
			}stat[0];
		}i2c_hs_rw_resp;
#define SUB_MAX_I2C_HS_RW_RESP_LONG (SUB_MAX_PACK_LONG-1-offsetof(sub_tag_t,i2c_hs_rw_resp.stat[0]))
...
#define SUB_MAX_I2C_SCAN (SUB_MAX_PACK-1-offsetof(sub_tag_t,i2c_scan_resp.slave_buf))
		...
		/* SPI Config2 */
		struct spi_config2_req_st
		{
			uint8_t		spi_SPCR;
			uint8_t		bbh_spi_SPCR;	/* BB on HW SPI bit:0..6=Tus/2 */
            uint8_t     reserved[8];
		}spi_config2_req;
		...
		{
			uint8_t		SS_config;
				#define	SUB_BB_SPI		0x80
				#define	SUB_BB_SPIx1	0x40
			uint8_t		sz;
			uint8_t		data[0];
		}spi_rdwr_req;
...
#define SUB_MAX_SPI_SDIO_RDWR (SUB_MAX_PACK-1-offsetof(sub_tag_t,spi_sdio_rdwr_req.data))
		...
		struct mdio_xfer_req_st
		{
			...
			struct mdio_xfer_st
			{
				...
				uint8_t	devad;		/* bit7:CFP_MSA   bit6,5:channel */
				uint8_t	data_lo;
				uint8_t	data_hi;
			}xfer[0];
		}mdio_xfer_req;
...
#define MAX_ADC_READS 30
		...
		/* Counter */
		struct cntr_config_req_st
		{
			uint8_t		_TCCR0B;	/* Bit7: - Clear */
			uint8_t		_TCCR1B;	/* Bit7: - Clear */
		}cntr_config_req;

		struct cntr_read_resp_st
		{
			uint8_t		cntr0[4];	/* LSB first */
			uint8_t		cntr1[4];	/* LSB first */
		}cntr_read_resp;
		...
		{
			uint8_t		UCSRC;	/* Bit7:RxEn, Bit6:TxEn, Bit0:UCSZn2 */
			uint8_t		UBRRL;
			uint8_t		UBRRH;	/* Bit7:U2X */
		}RS_config_req;
