...
*-------------------------------------------------------------------
* Version: $Id: sub_app.c,v 1.74 2012-03-30 12:57:32 avr32 Exp $
*-------------------------------------------------------------------
*/
#include <stdio.h>
#include <stdlib.h>
    ...
    */
    for( cmd=0; cmd<config.todo_sz; cmd++ )
    {
        ...
        case 'G':
            ...
            if( !rc )
                printf("SPI Config: %04xh (", tmp);
            (tmp & SPI_ENABLE     ) ? printf( "ENA " ) : printf("DIS ");
            (tmp & SPI_SLAVE      ) ? printf( "SLV " ) : printf("MTR ");
            (tmp & SPI_CPOL_FALL  ) ? printf( "FAL " ) : printf("RIS ");
            (tmp & SPI_SETUP_SMPL ) ? printf( "SET " ) : printf("SMP ");
            (tmp & SPI_LSB_FIRST  ) ? printf( "LSB " ) : printf("MSB ");
            switch( tmp & SPI_CLK_LOW )
            {
            case SPI_CLK_8MHZ:
                printf( "8MHz" ); break;
            case SPI_CLK_4MHZ:  
        ...
        case 'H':
            /* High Speed I2C rw */ 
            rc = sub_i2c_hs_rw( fd, config.mcode, config.hs_n, config.hs_xfer );
            if( ! rc )
            {
                for( i=0; i<config.hs_n; i++ )
                {
                    printf("R_W=%d St=0x%02X Sz=%03d(%03d)\n", 
                                        config.hs_xfer[i].r_w,
                                        config.hs_xfer[i].status,
                                        config.hs_xfer[i].act_sz,
                                        config.hs_xfer[i].sz );
                    hex_ascii_dump( config.hs_xfer[i].data, 
                                    config.hs_xfer[i].act_sz);
                }
            } 
            break;
        ...
        case 'Q':
            /* EEP Read */
            rc = sub_eep_read( fd, config.eep_addr, config.eep_buf, 
                                                        config.eep_sz );
            if( !rc )
                hex_ascii_dump( config.eep_buf, config.eep_sz );
            break;
        case 'q':
            /* EEP Write */
printf("EEP Write\n");
            rc = sub_eep_write( fd, config.eep_addr, config.eep_buf, 
                                                        config.eep_sz );
            break;

        case 'n':
            /* GPIOB Write */
            rc = sub_gpiob_write( fd, config.gpiob_val, &tmp1, 
                                                    config.gpiob_mask );
            if( !rc )
            {
...
int process_arg( int opt_index, char* equ_ptr )
{
    ...
    */
    ...
    case I2C_CMD_CASE:
    {
        ...
        {
            /* HS I2C */
            i = sscanf( equ_ptr+1, "%x,%d,%768c",
                        &config.hs_xfer[config.hs_n].sa,
                        &config.hs_xfer[config.hs_n].sz,
                        hex_buf );
            if( i == 2 )
            {
                config.hs_xfer[config.hs_n].r_w=1;
            }
            else
            {
                config.hs_xfer[config.hs_n].r_w=0;
                hex_scan( hex_buf, config.hs_xfer[config.hs_n].data, &sz );
                if( sz != config.hs_xfer[config.hs_n].sz )
                    goto i2cm1;
            }
            if( !config.hs_n )
                config.todo[config.todo_sz++]='H';
            config.hs_n++;
            break;
        }
        /* F/S I2C */
        ...
        if( i == 5 )
        {
            ...
            if( sz != config.sz )
            {
i2cm1:          printf("HEX size=%d mismatch in i2c option\n", sz);
                return -1;
            }
        }
        else if( i == 4 )
    ...
    case I2C_CFG_CMD_CASE:
        ...
        case 'm':
            ...
            break; 
        case 'M':   /* HS Master Code */
            config.mcode = strtoul(equ_ptr+1,0,0);
            break;
        case 'c':
        case 'C':
        ...
        while( *equ_ptr )
        {
            ...
            case 'h':
                ...
                if( *equ_ptr=='(' ) 
                {
                    ...
                    sscanf( equ_ptr+1, "%d", &i );
                    config.spi_config |= SPI_CLK_HZ(i);
                    equ_ptr = strchr( equ_ptr, ')' );
                    if( equ_ptr )
                        break;
                }
                printf( "Bad parameters for spi_cfg\n" );
                return -1;    
            }
            equ_ptr++;
        }         
...
xx1:
    ...
    case RS_TMG_CMD_CASE:
        if( toupper((int)equ_ptr[0]) == 'B' )
        {
            config.rs_flags |= RS_RX_BEFORE_TX;
            equ_ptr+=2;
        }
        else if( toupper((int)equ_ptr[0]) == 'A' )
        {
    ...
    case EEP_CMD_CASE:
    {
        ...
        i = sscanf(equ_ptr,"%d,%x,%768c", 
                &config.eep_sz, &config.eep_addr,hex_buf );
        if( i == 2 )
        {
            config.todo[config.todo_sz++]='Q';
        }
        else if( i == 3 )
        {
            hex_scan( hex_buf, config.eep_buf, &i );
            if( i != config.eep_sz )
            {
                printf( "Bad EEP hex size\n" );
                return -1; 
            }
            config.todo[config.todo_sz++]='q';
        }
